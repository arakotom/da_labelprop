...
class daLabelOT(object):
    def __init__(self, feat_extractor, data_classifier, phi, source_data_loader, target_bags, 
                 ...
                 use_div=False,proportion_S=None, do_adv = False,gamma=10,
                 epoch_train_phi=10,
                ):
        ...
        self.epoch_train_phi = epoch_train_phi
    ...
    def fit(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            self.train_phi = (epoch >= self.epoch_train_phi)
            ...
            for batch_idx in range(iterations):
                ...
                if self.align or self.train_phi:
                    ...
                    k_critic = 10 if epoch == self.epoch_start_align else 3

                    if not self.do_adv:
                        for i_k in range(k_critic):
                            ...
                            for i_c in range(self.n_class):
                                ...
                                n_in_class = len(ind)          
                                a[ind] = bag_prop_w[i_c]/n_in_class if n_in_class > 0 else 0.0
                            b /= torch.sum(b)
                            a /= torch.sum(a)
                            b = b.float()
                            a = a.float()
                            ...
                            # barycentric mapping
                            zs_map = torch.mm(gamma,zt_nograd)*(1/(gamma@torch.ones(n_2).to(self.device))).reshape(-1,1)
                        ...
                        for _ in range(k_critic):
                             #(x_s_w, y_s_w), (bag_t_w, _) = next(batch_iterator_w)
                            (x_s_w, y_s_w), (bag_t_w) = next(batch_iterator_w)
                            x_t_w = bag_t_w['data']
                            y_t_w = bag_t_w['label']
                            bag_prop_w = bag_t_w['prop']
                            ...
                            for j in range(self.n_class):
                                ...
                                if nb_sample != 0:
                                    source_weight_un_w[y_s_w == j] = bag_prop_w[j] / nb_sample

                            with torch.no_grad():
                                z_w = self.feat_extractor(torch.cat((x_s_w, x_t_w), 0))
                                # mapping the source domain
                                s_w = self.phi(z_w[:x_s_w.shape[0]])[0]
                                t_w = z_w[x_s_w.shape[0]:]
                                # take minimum length of the two tensors
                                if s_w.size(0) > t_w.size(0):
                                    ...
                                    t_w = t_w[ind]
                            #print(s_w.size(), t_w.size())
                            gp = gradient_penalty(self.domain_classifier, s_w, t_w, self.cuda)
                            critic_w = self.domain_classifier(torch.cat((s_w, t_w), 0))
                            wasserstein_distance_w = (critic_w[:s_w.shape[0]] * source_weight_un_w.detach()).sum() - critic_w[s_w.shape[0]:].mean()
                            critic_cost = - wasserstein_distance_w + self.gamma * gp
...
if __name__ == '__main__':
    ...
    dalabelot = daLabelOT(feat_extract_dalabelot, data_class_dalabelot, phi_dalabelot, source_loader, target_bags,
                    ...
                    do_adv=False, domain_classifier=domain_class_dalabelot)
